...
export default function Contact() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>): void => {
    ...
    const emailSubject = "New contact request from portfolio";
    const emailBody = `Hello Tousif,\n\nI am interested in your services.\n\nName: ${formData.name}\nEmail: ${formData.email}\n\nMessage:\n${formData.message}`;
    const mailtoURL = `mailto:ertousif49@gmail.com?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(emailBody)}`;
  ...
  return (
    <section id="contact" className="relative min-h-screen py-16 px-4 sm:px-6 lg:px-8 bg-[#020617] overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.8 }} className="text-center mb-12 sm:mb-20">
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-4 leading-tight">Contact</h2>
          <p className="text-gray-400 text-base sm:text-lg max-w-2xl mx-auto">
            Have a project in mind? Let&apos;s build something great together.
          </p>
          <div className="mt-4 flex flex-wrap justify-center gap-4 text-sm text-gray-500">
            <span>📧 ertousif49@gmail.com</span>
            {/* <span>📞 +91-6204190723</span> */}
        ...
        <div className="flex justify-center px-2">
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            transition={{ duration: 0.8 }} className="w-full sm:w-[90%] md:w-[80%] lg:w-[60%]">
